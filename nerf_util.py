...
def sample_dists(ray_size, dist_range, intvs, stratified):
    ...
    if stratified:
        rands = jt.rand(batch_size, num_rays, intvs, 1)
    else:
        rands = jt.empty(batch_size, num_rays, intvs, 1).fill_(0.5)
    rands += jt.arange(intvs, dtype=jt.float)[None, None, :, None]  # [B,R,N,1]
...
def sample_dists_from_pdf(bin, weights, intvs_fine):
    ...
    Returns:
        dists (tensor [batch_size, num_ray, intvs, 1]): Sampled distance for all rays in a batch.
    """
    pdf = jt.normalize(weights + 1e-12, p=1, dim=-1)
    # Get CDF from PDF (along last dimension).
    cdf = pdf.cumsum(dim=-1)  # [B,R,N]
    cdf = jt.concat([jt.zeros_like(cdf[..., :1]), cdf], dim=-1)  # [B,R,N+1]
    # Take uniform samples.
    grid = jt.linspace(0, 1, intvs_fine + 1)  # [Nf+1]
    unif = 0.5 * (grid[:-1] + grid[1:]).repeat(*cdf.shape[:-1], 1)  # [B,R,Nf]
    idx = jt.searchsorted(cdf, unif, right=True)  # [B,R,Nf] \in {1...N}
    # Inverse transform sampling from CDF.
    low = (idx - 1).clamp(min_v=0)  # [B,R,Nf]
    high = idx.clamp(max_v=cdf.shape[-1] - 1)  # [B,R,Nf]
...
def ray_generator(pose, intr, image_size, num_rays, full_image=False, camera_ndc=False,
    ...
    if ray_indices is None:
        ...
        if full_image:
            ...
            ray_indices = jt.arange(0, num_pixels).repeat(batch_size, 1)  # [B,HW]
        else:
            # Sample rays randomly. The below is equivalent to batched jt.randperm().
            ray_indices = jt.rand(batch_size, num_pixels).argsort(dim=1)[:, :num_rays]  # [B,R]
    ...
    for c in range(0, ray_indices.shape[1], num_rays):
        ...
        batch_idx = jt.arange(batch_size).repeat(ray_idx.shape[1], 1).t()  # [B,R]
...
def slice_by_ray_idx(var, ray_idx):
    ...
    batch_idx = jt.arange(batch_size).repeat(num_rays, 1).t()  # [B,R]
...
def positional_encoding(input, num_freq_bases):
    ...
    """
    freq = 2 ** jt.arange(num_freq_bases, dtype=jt.float32) * np.pi  # [L].
...
class MLPwithSkipConnection(jt.nn.Module):
    ...
    def __init__(self, layer_dims, skip_connection=[], activ=None, use_layernorm=False, use_weightnorm=False):
        ...
        self.linears = jt.nn.ModuleList()
        if use_layernorm:
            self.layer_norm = jt.nn.ModuleList()
        ...
        for li, (k_in, k_out) in enumerate(layer_dim_pairs):
            ...
            if use_layernorm and li != len(layer_dim_pairs) - 1:
                self.layer_norm.append(jt.nn.LayerNorm(k_out))
            if li == len(layer_dim_pairs) - 1:
                jt.init.constant_(self.linears[-1].bias, 0.0)
...
def intersect_with_sphere(center, ray_unit, radius=1.0):
    ...
    outside = b2_minus_4ac < 0.0
    b2_minus_4ac[outside] = 0.0
    dist_near = -ctv - b2_minus_4ac.sqrt()
    dist_far = -ctv + b2_minus_4ac.sqrt()
    return outside, dist_near, dist_far
...
def contract(x, r_in=1, r_out=2, eps=1e-8):
    ...
    x_contract = scale * jt.normalize(x, dim=-1)  # [...,3]
    # No effect if within r_in.
    inside = x_norm <= r_in
    x_warp = jt.where(inside, x, x_contract)  # [...,3]
    return x_warp
...
def contract_jacobian(x, r_in=1, r_out=2, eps=1e-8):
    ...
    eye = jt.init.eye(3).repeat(*x_norm.shape)  # [...,3,3]
    ...
    # No effect if within r_in.
    inside = x_norm <= r_in
    jacobian = jt.where(inside, eye, jacobian_contract)  # [...,3]
    return jacobian
